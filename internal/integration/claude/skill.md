...
Use MCP tools through Claude Code following the three-tier knowledge protocol.
...
Always set `analyzer: "claude-code"` for storeAnalysis and storeFeature.
...
- **Analyzer Field**: Always `"claude-code"` — this identifies your analysis across sessions
...
6. "Deep dive on User Auth feature"
   → `storeFeature(project_id, analyzer: "claude-code", name: "User Authentication", implementation_status: "complete", feature_architecture: "JWT middleware + session store", pattern: "Middleware-based auth")`
